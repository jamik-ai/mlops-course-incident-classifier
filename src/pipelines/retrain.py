...
def retrain_pipeline(training_data_path: str = "data/current/current_dataset.csv", model_dir: str = "models") -> dict:
    mlflow.set_tracking_uri(MLFLOW_TRACKING_URI)
    mlflow.set_experiment("call-volume-forecast-retraining")
    df = _read_training_data(Path(training_data_path))
    ...
    with mlflow.start_run(run_name="retrain_xgboost") as run:
        ...
        mlflow.xgboost.log_model(xgb_model=model, artifact_path="model", registered_model_name=MODEL_NAME)
